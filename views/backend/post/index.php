...
$option = $_REQUEST['option'] ?? '';
                $all=Post::where('status','!=','0')->where('post.type','!=','page')->count();
                ...
                {
                    $key = $_POST['search'];
                    $list_post = Post::join('topic','topic.id','=','post.topic_id')->where('post.status','!=','0')->where('post.type','!=','page')
        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between mb-3">
                    ...
                    <div><a href="?option=post&cat=create" class="btn bg-primary rounded-0 "><i class="fa-solid fa-plus"></i></a><button
            ...
            <div class="card-body">
            <form action="?option=post&cat=process" method="post" enctype="multipart/form-data">
                            ...
                            <tr>
                                ...
                                <td class="text-right">
                                    ...
                                    <a title="Tắt" class="btn btn-primary"
                                        href="index.php?option=post&cat=status&sort=<?=$sort?>&page=<?=$j?>&id=<?=$value->id?>">
                                        <i class="fa-solid fa-toggle-on"></i></a>
                                    <?php else:?>
                                    <a title="Bật" class="btn btn-danger"
                                        href="index.php?option=post&cat=status&sort=<?=$sort?>&page=<?=$j?>&id=<?=$value->id?>"><i
                                            class="fa-solid fa-toggle-off "></i></a>
                                    <?php endif;?>
                                    <a title="sửa" class="mx-2 btn btn-warning"
                                        href="index.php?option=post&cat=edit&id=<?=$value->id?>"><i
                                            class="fa-solid fa-pen-to-square"></i></a>
                                    <a title="xem" class="mr-2 btn btn-primary"
                                        href="index.php?option=post&cat=show&id=<?=$value->id?>"><i
                                            class="fa-solid fa-eye"></i></a>
                                    <a title="Xóa vào thùng rác" class="btn btn-danger mr-2"
                                        href="index.php?option=post&cat=delete&sort=<?=$sort?>&page=<?=$j?>&id=<?=$value->id?>"><i
                ...
                <div class="d-flex justify-content-between mb-3">
                    ...
                    <div>
                        <nav aria-label="...">
                            <ul class="pagination">
                                <li class="page-item">
                                    <span class="page-link"><a href="?option=post&sort=<?=$sort ?>&page=1">Đầu</a></span>
                                </li>
                                <?php for($i=1;$i<=$number_of_page;$i++): ?>
                                <li class="page-item"> <a class=' page-link <?php if($j==$i)echo 'bg-primary' ?>'
                                        href='?option=post&sort=<?=$sort ?>&page=<?=$i?>'><?=$i?></a></li>
                                <?php endfor; ?>
                                <li class="page-item">
                                    <a class="page-link" href="?option=post&sort=<?=$sort ?>&page=<?=--$i?>">Cuối</a>
                                </li>
                            </ul>
                        </nav>
                    </div>
                    <div><a class="btn bg-primary rounded-0 " href="index.php?option=post&cat=recycle&page=1">Thêm</a>
                        <a class="btn bg-danger rounded-0 " href="index.php?option=post&cat=recycle&page=1">Thùng
                            rác</a>
                    </div>
                </div>
                <!-- /.card-footer-->
