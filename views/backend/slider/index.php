...
<?php
                $all=Slider::where('status', '!=',0)->count();
                ...
                {
                    $key = $_POST['search'];
                    $list_slider = Slider::where('status', 1)->where('name','like',"%".$key.'%')->get();
} else {
        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex justify-content-between mb-3">
                    ...
                    <div><a href="?option=slider&cat=create" class="btn bg-primary rounded-0 "><i class="fa-solid fa-plus"></i></a><button
                    ...
                    <table class="table table-hover ">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <img width="100px" height="50px" src="../public/image/slide/<?=$value->image?>"
                                        alt="<?=$value['name']?>">
                                </td>
                                ...
                                <td class="text-right d-flex">
                                    ...
                                    <a title="Tắt" class="btn btn-primary"
                                        href="index.php?option=slider&cat=status&sort=<?=$sort?>&page=<?=$j?>&id=<?=$value->id?>">
                                        <i class="fa-solid fa-toggle-on"></i></a>
                                    <?php else:?>
                                    <a title="Bật" class="btn btn-danger"
                                        href="index.php?option=slider&cat=status&sort=<?=$sort?>&page=<?=$j?>&id=<?=$value->id?>"><i
                                            class="fa-solid fa-toggle-off "></i></a>
                                    <?php endif;?>
                                    <a title="sửa" class="mx-2 btn btn-warning"
                                        href="index.php?option=slider&cat=edit&id=<?=$value->id?>"><i
                                            class="fa-solid fa-pen-to-square"></i></a>
                                    <a title="xem" class="mr-2 btn btn-primary"
                                        href="index.php?option=slider&cat=show&id=<?=$value->id?>"><i
                                            class="fa-solid fa-eye"></i></a>
                                    <a id="liveToastBtn" title="Xóa vào thùng rác" class="btn btn-danger mr-2"
                                        href="index.php?option=slider&cat=delete&sort=<?=$sort?>&page=<?=$j?>&id=<?=$value->id?>"><i
                            ...
                            <ul class="pagination">
                                <li class="page-item">
                                    <span class="page-link"><a
                                            href="?option=slider&page=1">Đầu</a></span>
                                </li>
                                <?php for($i=1;$i<=$number_of_page;$i++): ?>
                                <li class="page-item"> <a class=' page-link <?php if($j==$i)echo 'bg-primary' ?>'
                                        href='?option=slider&page=<?=$i?>'><?=$i?></a></li>
                                <?php endfor; ?>
                                <li class="page-item">
                                    <a class="page-link" href="?option=slider&page=<?=--$i?>">Cuối</a>
                                </li>
                            </ul>
                        </nav>
                    </div>
                    <div><a class="btn bg-primary rounded-0 " href="?option=slider&cat=create">Thêm</a>
                        <a class="btn bg-danger rounded-0 " href="index.php?option=slider&cat=recycle&page=1">Thùng
                            rác</a>
                    </div>
